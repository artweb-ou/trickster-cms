...
window.FullScreenGalleryCenterComponent = function (galleryComponent, galleryObject) {
  ...
  var createDomStructure = function () {
    ...
    for (var i = 0; i < imagesObjectsList.length; i++) {
      var imageComponent = new FullScreenGalleryImageComponent(imagesObjectsList[i], self, galleryObject.getDescriptionType())
...
window.FullScreenGalleryImageComponent = function (imageInfo, parentObject, descriptionType) {
  var self = this
