...
class submitRegistration extends structureElementAction
{
    ...
    public function execute(&$structureManager, &$controller, &$structureElement)
    {
        ...
        $emailToCheck = false;
        if ($this->validated && !isset($coreFields['userName'])) {
            if (isset($coreFields['email'])) {
                $emailToCheck = $coreFields['email']->value;
                $coreFields['userName'] = $coreFields['email'];
            } else {
                $this->validated = false;
            }
        }
        ...
        if ($this->validated && ($newUser || $user->userName != $coreFields['userName']->value)) {
            if ($user->checkExistance($coreFields['userName']->value)) {
                $this->validated = false;
                    ...
                    if ($userElement) {
                        ...
                        if ($userElement->importExternalData($mainData)) {
                            ...
                            if ($newUser && isset($coreFields['password'])) {
                                if ($userId = $user->checkUser($userElement->userName, $coreFields['password']->value)) {
                            ...
                            $visitorsManager->updateCurrentVisitorData(['email'  => $user->email,
                                                                        'userId' => $user->id
                            ]);

                            $userElement->checkSubscription($structureElement->subscribe);
                            $this->ajaxFormSuccess = true;
                        }
                        if ($newUser && $structureElement->getFirstParentElement()->structureType == 'shoppingBasket') {
                            $this->ajaxFormSuccessRedirection = $controller->fullURL;
                        }
                    }
                }
            }
