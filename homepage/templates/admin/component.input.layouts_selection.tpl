            ...
            {foreach $item.layouts as $layout}
                <div class="form_table_layout_item">
                    <label class="form_table_layout_item_label">
                        ...
                        {/if}
                        <div class="form_table_layout_item_text">{if $fieldName == 'colorLayout' && $layout != 'none'}<small>#{$layout|lower}</small>{elseif $fieldName == 'colorLayout' && $layout == 'none'}<small>{translations name="shared.color_layout_none"}</small>{else}{translations name="{$translationGroup}.layout_{$layout}"}{/if}</div>
