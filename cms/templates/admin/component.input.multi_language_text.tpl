{foreach from=$formData.$fieldName key=languageId item=object}
    <div class="form_items{if !empty($item.class)} {$item.class}{/if}{if $formErrors.$fieldName.$languageId} form_error{/if}" {if !empty($item.style)}style="{$item.style}{/if}">
        ...
        <div class="form_field">
            <input class='input_component' min="{if !empty($item.minValue)}{$item.minValue}{/if}"{if !empty($item.maxValue)} max="{$item.maxValue}"{/if} type="{if !empty($item.inputType)}{$item.inputType}{else}text{/if}" value="{$object}" name="{$formNames.$fieldName.$languageId}" />
        </div>
